...
### SPL Monitor — 10EaZy-style remote portal (Smaart API)
*Last updated: July 1, 2026*

- Live at **https://spl.tinydoorstudios.com**. Runs on the n8n VM (192.168.200.84) as systemd `spl-monitor` — see [[spl-monitor-host]]. Asset version: v=21.
- **Current features:** Hero (10s LAeq + headroom LED bar) · traffic light (yellow=85/red=90 on LAeq 10s) · strikes row (passcode reset: 2578) · tiles (Instant / 6-min compliance / Prediction / Davidson C-A) · All Metrics strip · 8 virtual positions · Sound Ordinance (FOH + 3 estimated, 75 dBA limit) · chart · **show/engineer banner** (new, see below). Full layout fits one screen.
- Compliance: native `LAeq 6`. Violation trigger: `LAeq 10s` ≥ 90.
- Smaart box: 192.24.143.121:26000 (DiGiCo UB MADI ASIO / SPL).
- **Critical .env: SPL_PORT=8090** — Cloudflare tunnel routes to 8090, not config.json default 8080. Full required env vars in `~/.claude/CLAUDE.md`.
- **Show/Engineer banner + nightly email (2026-07-01):** dashboard header now shows tonight's Fountain Square show and mix engineer, pulled from three of Brian's public Google Sheets (crew schedule, crew-code lookup, band booking) — no API key needed, all view-only "publish to web" CSVs. New module `backend/showinfo.py`; config in `config.json` under `showInfo`. Same data now leads the nightly-summary email (Show/Location/Engineer block, subject line). Full writeup in `Code/SPL-Monitor/docs/DEPLOYED.md`. Depends on the VM having outbound HTTPS to `docs.google.com` — fails soft (fields go blank, logged) if that's ever blocked.
- **Next:** Cloudflare Access gate · PM2 update (v6→v7) · extend show-info to Memorial Hall once that venue comes off `comingSoon` · the non-technical dashboard guide (below) doesn't yet cover the new banner — screenshots exist but a new page hasn't been added.
- **Non-technical dashboard guide** for bosses: `spl_dashboard_guide_v2.pdf` (7 pages — cover, full-dashboard orientation page, then the 5 walkthrough sections). Lives in `audio/SOP Stuff/FSQ/SPL Monitor Dashboard Guide/` (source `.html` + screenshots also kept there) and mirrored in `Code/SPL-Monitor/docs/`. Real screenshots only, Simulator-mode data, two limits kept distinct (90 dBA internal vs. 75 dBA city ordinance).
...
### 2026-07-09 — hot-mag A/B eval + "KB is for longevity, not research" guardrail (from session 2026-07-08/09)
- A/B eval of the two blind Hot Magnolias rebuilds delivered: **Fable 5 (hot-mag 3) won over Opus 4.8 (hot-mag 2)** — the gap was research discipline, not EQ taste. Fable web-sourced all 11 research units with named sources + quantitative capsule facts; Opus web-researched 3 and leaned on the KB for "familiar" mics, which produced the two likely soundcheck corrections (i5 +2 @ 5k on a capsule that already bakes in +9 @ 5.5k; boosted lows on both kick mics against its own note), wrote three near-identical horn EQs after saying it would slot them, and assumed dry July air when the real forecast was 74% RH. Both runs clean on the hard guardrails; .ses files byte-compared — A/B was clean.
- Deliverables in `Fountain Square/hot-mag A-B eval/`: the eval, `PIPELINE-UPGRADE-FOR-OPUS-4.8.md` (11 anchored edits to `_skills/eq-advisor`, `_skills/show-deep-build`, `NEW-SHOW.md`, and the KB pipeline article, with an acceptance test hot-mag 3 passes and hot-mag 2 fails), and a run guide with word-for-word FSQ/Memo prompts that work on any model.
- **New standing rule from Brian (explicit): the KB is for longevity/verification, never a research source.** No model may justify an EQ value with "KB only" — every research unit gets a fresh web pass with a named external source; the KB's build-time job is cross-checking and conflict resolution. Nuances kept intact: console-verified past shows still count as evidence beside fresh research (2026-07-05 rule), and a web↔KB disagreement still stops the build to ask.
- Guardrail is currently written into the upgrade MD + run-guide prompts only — the skill files themselves are NOT yet edited. Next step: Brian feeds `PIPELINE-UPGRADE-FOR-OPUS-4.8.md` to Opus to apply the edits, then the acceptance test on the next build confirms it took. (Tracked in active-projects.md Open Issues.)
- Same day: the hot-mag 2 rebuild initially ran a stale plugin-cache copy of `build_packet.py` (reverb section missing from the output) — canonical script lives in `audio/_skills/`; rebuilt clean, gotcha logged to `_system/IMPROVEMENTS.md`.
...
### Session — 2026-07-09 (skill consolidation)
- Brian asked for an eval of eq-advisor / show-deep-build / NEW-SHOW.md with redundancy removed and one skill for the whole pipeline. Merged eq-advisor INTO show-deep-build (Part II = "The EQ method"; standalone EQ questions run it alone). Old eq-advisor archived (`_skills/_ARCHIVE/eq-advisor-retired-2026-07-09/`); NEW-SHOW.md slimmed to a router + don't-forgets; harvest detail moved into the skill.
- Applied ALL 11 pending edits from `PIPELINE-UPGRADE-FOR-OPUS-4.8.md` into the merged skill (research floor, capsule-voicing gate, two-mic lanes, section slotting, 3-option web↔KB forks, fetched weather, factory-anchored reverbs, carried flags, numeric dynamics, hardware-state flags) + NEW-SHOW.md + KB pipeline article. Upgrade doc marked APPLIED (do not re-run it). Acceptance test = next deep build vs hot-mag 3 spec.
- Fights found: Cowork-installed skill copies were stale snapshots (eq-advisor 06-23, show-deep-build 06-25 — no July rules). New `_skills/show-deep-build.skill` package built; Brian must delete both installed copies in Cowork settings and upload the new one. Also fixed NEW-SHOW.md's stale ".md + .html + PDF via weasyprint" packet claim (build_packet.py is reportlab, no HTML since 2026-07-06).
- Updated: all 3 CLAUDE.md files, active-projects.md (upgrade item → applied), KB CHANGELOG, IMPROVEMENTS.md, auto-memory deep-think-default.
