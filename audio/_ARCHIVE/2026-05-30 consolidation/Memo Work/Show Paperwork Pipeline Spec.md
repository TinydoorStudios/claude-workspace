...
~/Documents/Claude/audio/workflow start files/eq-starting-points.md              ← EQ starting points by instrument
...
An Excel input list plus optional genre/influences text enters the pipeline and produces deliverables in two stages. Stage 1 produces the Show Document as three parallel files: an MD (machine-readable patcher source), a combined HTML (Show Packet sections first, then FOH Channel Processing sections), and a PDF rendered from the HTML via weasyprint. The Show Document combines the production overview (band context, stage patch, monitor sends, console reference, watchpoints) with the full per-channel EQ/dynamics reference in one master file. Stage 2, on explicit request only, produces a show-specific Python patcher script and the DiGiCo Quantum 225 `.ses` file generated from it. All files land in the show folder under the venue root. Nothing is built speculatively — each stage waits for a request.
...
## 3. Step 1 — Show Document (Combined)

### Three output files, always together
```
ShowName - FOH Channel Processing.md
ShowName - Show Document.html
ShowName - Show Document.pdf
```
Produce all three simultaneously. The PDF is rendered from the HTML:
```bash
weasyprint "ShowName - Show Document.html" "ShowName - Show Document.pdf"
```
Do not use reportlab. It does not produce output that matches the locked format.

The MD remains a standalone file (patcher source only). The HTML and PDF are the combined master document.

### Combined HTML structure (locked — The Brit Pack 2026-05-28 Show Document is the reference)

**Part 1: Show Packet** (show packet sections first)
1. Cover block — show title (navy `#1c3558` bg, gold `#c8a84b` subtitle), venue/date/console/channel counts, gold rule
2. Band Context card — band description, personnel, sonic target sentence
3. Stage Patch — two-column table (one column per stage box): Drop # | Source | Mic/DI | → Console Ch | 48V. 48V rows highlight red (`#fff0f0`). Footnote for channels with no stage box drop.
4. Monitor Sends — reference table: Send # | Position / Description. No mix priority suggestions.
5. Console Reference — two tables: channel labels (CH | Short Label | Instrument | Mic/DI | 48V | HPF Hz | Group) and VCA groups (VCA # | Name | Channels)
6. Show Notes & Watchpoints — flagged note blocks: red = safety-critical, blue = routing/workflow, dark gray = contextual

**Page break**

**Part 2: FOH Channel Processing**
7. Title: show name, console, venue, date
8. Input list summary table: Ch | Instrument | Mic/DI
9. Per-channel cards in channel order (see card spec below)
10. Global reverb section — Seventh Heaven Pro presets, tuned to genre and room (Memo RT60 ≈ 1.6 s)
11. Critical notes — reverberant-room reminders, Memo problem frequencies, polarity warnings, stereo pair linking
12. Workflow reminders — VCA groups, manual-entry channels, anchor channel for the mix

**File Index** (always last, regardless of section order)
13. File index — one row per file in the show folder. Columns: filename | description. Includes this document itself. Updated to reference the combined Show Document, not separate Show Packet and Channel Processing files.

### Section order
Current standard: Show Packet first, Channel Processing second. Order may change per show if Brian specifies.
...
### Reverb section
Always use real Seventh Heaven Pro preset names (see userMemory). Format:
`Preset: [Name] | Decay Xs • PreDelay Xms • Early/Late XX/XX • HF Damp X.X • Mix X% — [rationale]`
Include return EQ sweetening (HS, LC values). Tune preset, decay, and pre-delay to genre. In the Memo (RT60 ≈ 1.6 s), start conservative — the room is doing reverb work already.
...
## 6. Show Packet Content — Spec

The show packet is now integrated into the combined Show Document (see Section 3). It is not a separate deliverable. This section documents the show packet content spec for reference when building Part 1 of the combined document.

### Show packet sections
1. **Cover / Band Context** — show title, venue, date, console, channel/send counts. Band description paragraph from genre research. Sonic target sentence.
2. **Stage Patch** — two-column layout: one column per stage box. Each row: Drop # | Source | Mic/DI | → Console Ch | 48V flag. 48V rows highlight red (`#fff0f0`). Handheld wireless channels with no stage box drop get a footnote.
3. **Monitor Sends** — reference table only: Send # | Position / Description. No mix priority suggestions. The engineer sets mix content at soundcheck.
4. **Console Reference** — two tables: (a) channel labels: CH | Short Label | Instrument | Mic/DI | 48V | HPF Hz | Group; (b) VCA groups: VCA # | Name | Channels.
5. **Show Notes & Watchpoints** — flagged note blocks. Colors: red = safety-critical (phantom power, ribbon mics), blue = routing/workflow decisions, dark gray = contextual notes. Cover anything load-in-critical not obvious from the input list: unusual patching, dual vocal positions, split topology, IEM confirmation items, monitor position notes.

### What the show packet is NOT
It is not a duplicate of the channel processing section. It does not contain EQ values, DEQ parameters, dynamics settings, or Quick Summary paragraphs. Those live in Part 2. The show packet content is a load-in and patching reference.

### File Index
Moves to the very end of the combined document, after the channel processing section. Updated to list the combined Show Document, not separate Show Packet and Channel Processing files.
...
Combined HTML:    ShowName - Show Document.html
Combined PDF:     ShowName - Show Document.pdf
...
The separate `ShowName - FOH Channel Processing.html/pdf` and `ShowName - Show Packet.html/pdf` files are no longer standard outputs. The combined Show Document replaces both.

---

## 8. Output Order and Delivery

1. **Stage 1 — Show Document**: present MD, HTML, and PDF links simultaneously. All three or none.
