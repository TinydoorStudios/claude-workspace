...
~/Documents/Claude/audio/Live Sound KB/Wiki/INDEX.md               ← KB map
~/Documents/Claude/audio/workflow start files/Show Processing Quick Reference.md ← this file
```

The Live Sound KB at `~/Documents/Claude/audio/Live Sound KB/` is the canonical reference for venues, consoles, mic library, EQ starting points, reverb reference, and workflow detail. Read the INDEX at startup to know what's available, then pull specific articles as the task requires. Do not read all articles at startup.

The old individual files (venue-notes.md, mic-go-tos.md, eq-starting-points.md, console-reference.md, reverb-reference-memo.md, venue-notes-3CDC.md) are now superseded by the KB. Keep them for now but treat the KB as the authoritative version going forward.
...
```
Preset: [Bank / Name] | Decay Xs • PreDelay Xms • VLF [dB or off] • E/L [Early↑ / Balanced / Late↑] • Late Rolloff [kHz] • Mix X% — [rationale]
```
Include return EQ (HS, LC). In the Memo, start conservative — the room adds reverb naturally.

Key controls: VLF is a single level slider (sub-200Hz reverb body — cut hard at Memo, standing waves). Early/Late is a single balance slider, not two values. Late Rolloff ≠ HF Damp — separate controls for early and late exist. Pre-delay moves late reverb and VLF only; early reflections are unaffected. See reverb-reference-memo article for full control reference.
