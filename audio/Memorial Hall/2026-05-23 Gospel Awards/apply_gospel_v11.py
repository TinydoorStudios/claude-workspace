#!/usr/bin/env python3
"""
Gospel Awards 2026 — DiGiCo Q225 .ses patcher  v11

What this does
--------------
Takes the working template (`brian memo v2-99072ff1.ses`) and applies the
Gospel Awards 2026 show config on top of it: channel names, HPF/LPF, four
EQ bands per channel, gate, comp. 23 channels touched (1–22, 24); channel
23 is left alone.

Why no Mustard logic
--------------------
v1 through v10 actively touched Mustard tags trying to ensure Mustard was
off on every channel. That was the bug. The template already has Mustard
fully off — the 1.0 values in tags 0x0503 / 0x050e / 0x0511 / 0x08e1 /
0x08e8 / 0x0ee8 / 0x0efe / 0x1d47 represent the off state, not "enabled."
Zeroing them was what produced "Mustard Dynamic 2 ON" on the console
after every load (v10's "flipped" wording was the giveaway).

v11 leaves every byte in the Mustard regions untouched. The post-write
verification asserts this explicitly: every Mustard tag record in the
output file matches the source file byte-for-byte.
...
all other parameters are tagged TLV records found by scanning the strip.
...
# ── Mustard tags — listed only for the post-write verification ──────────────
# Nothing in this script writes to these. The verification step confirms that
# every record carrying one of these tags is identical between input and
# output. If that ever fails, we've reintroduced the v1–v10 bug.

MUSTARD_TAGS = (
    0x0503,   # Dyn master
    0x050e,   # Dyn bands (D1/D2/D3)
    0x0511,   # Dyn section
    0x08e1,   # Voc A
    0x08e8,   # Voc B
    0x0ee8,   # extended
    0x0efe,   # extended
    0x1d47,   # Dyn1 secondary
)
...
def apply_gate(data, strip_num, enabled, open_thr, close_thr,
               attack, release, range_db):
    write_tag(data, strip_num, TAG_GATE_ENABLE,       0, ON if enabled else OFF)
    write_tag(data, strip_num, TAG_GATE_OPEN_THRESH,  0, open_thr)
    write_tag(data, strip_num, TAG_GATE_CLOSE_THRESH, 0, close_thr)
    write_tag(data, strip_num, TAG_GATE_ATTACK,       0, attack)
    write_tag(data, strip_num, TAG_GATE_RELEASE,      0, release)
    write_tag(data, strip_num, TAG_GATE_RANGE,        0, range_db)


def apply_comp(data, strip_num, enabled, thresh, release, makeup):
    write_tag(data, strip_num, TAG_COMP_ENABLE,  0, ON if enabled else OFF)
    write_tag(data, strip_num, TAG_COMP_THRESH,  0, thresh)
    write_tag(data, strip_num, TAG_COMP_RELEASE, 0, release)
    write_tag(data, strip_num, TAG_COMP_MAKEUP,  0, makeup)


def apply_channel(data, strip_num, name, old_name, hpf, lpf,
                  bands, gate_data, comp_data):
    print(f"  Strip {strip_num:2d}  {name}")
    ...
    if gate_data:
        apply_gate(data, strip_num,
                   enabled=gate_data['en'],
                   open_thr=gate_data['open_thr'],
                   close_thr=gate_data.get('close_thr', gate_data['open_thr'] - 3),
                   attack=gate_data['atk'],
                   release=gate_data['rel'],
                   range_db=gate_data['range'])

    apply_comp(data, strip_num,
               enabled=comp_data['en'],
               thresh=comp_data['thr'],
               release=comp_data['rel'],
               makeup=comp_data['makeup'])
...
def G(open_thr, atk, rel, range_db):
    return dict(en=True, open_thr=open_thr, close_thr=open_thr - 3,
                atk=atk, rel=rel, range=range_db)


def C(thr, rel, makeup):
    return dict(en=True, thr=thr, rel=rel, makeup=makeup)


# ── channel table ────────────────────────────────────────────────────────────
# strip_num: (name, old_name, hpf_hz, lpf_hz, [bands], gate|None, comp)
...
CHANNELS = {
    ...
    17: ("BG Vox 1",    "17", 100,  15000,
         [B(+3,  8000, 0.7, SHELF), B(+4, 3000, 1.2, BELL),
          ...
          B(-4,   200, 1.5, SHELF)],
         G(-40, 0.005, 0.100, 70),  C(-14, 0.100, +3)),
    ...
    24: ("Podium",      "24", 120,  12000,
         [B(+2,  6000, 0.8, SHELF), B(+4, 2500, 1.2, BELL),
          ...
          B(-5,   200, 1.5, SHELF)],
         G(-50, 0.008, 0.400, 40),  C(-16, 0.150, +3)),
}


# ── verification ─────────────────────────────────────────────────────────────

def verify_mustard_untouched(src_bytes, out_bytes):
    """
    Walk the file. For every TLV record whose tag matches one in MUSTARD_TAGS,
    the float value at that offset must be identical between src and out.
    """
    assert len(src_bytes) == len(out_bytes), "size mismatch"
    mustard_sigs = {struct.pack('<H', t) for t in MUSTARD_TAGS}
    diffs = []
    # TLV records are 8 bytes: [float4][tag2][bidx2]; tag bytes live at i+4..i+6
    for i in range(0, len(src_bytes) - 7):
        if src_bytes[i + 4:i + 6] in mustard_sigs:
...
def spotcheck(src_bytes, out_bytes):
    ...
    diffs = verify_mustard_untouched(src_bytes, out_bytes)
    print(f"\nMustard verification:")
    if not diffs:
        print(f"  PASS — every Mustard tag value is byte-identical to template.")
    else:
        print(f"  FAIL — {len(diffs)} Mustard records were modified:")
...
def main(argv=None):
    ...
    for strip_num in sorted(CHANNELS.keys()):
        name, old_name, hpf, lpf, bands, gate_data, comp_data = CHANNELS[strip_num]
        apply_channel(data, strip_num, name, old_name, hpf, lpf,
                      bands, gate_data, comp_data)
