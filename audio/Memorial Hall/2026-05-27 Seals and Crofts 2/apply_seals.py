...
v2 channel layout (25 channels — keyboards dropped):
  Ch 1–14   unchanged from v1
  Ch 15–16  Tracks L/R  (was Keys L/R)
  Ch 17     Click       (was Tracks L)
  Ch 18     Video       (was Tracks R)
  Ch 19–22  Vocals (Ziggy, Lua, Brady, Key)
  Ch 23–24  Brian Vox, John Vox  (now within 24-strip patcher range)
  Ch 25     Test / SM57

Always run against the original template (brian memo v2.ses), not a
previously patched file — old_names are template strip numbers ("1"–"25").
"""
...
# ── channel table — Seals and Crofts 2 (v2, 25 channels) ─────────────────────
# Source: Seals and Crofts 2 - FOH Channel Processing.md
# old_name = template strip number (run against brian memo v2.ses only)
...
CHANNELS = {
    ...
    15: ("Tracks L",   "15",  80,   OFF_LPF, [     # was Keys L
        FLAT(),
        FLAT(),
        B(-3,  315, 2.0, BELL),
        B(-2,  200, 1.5, SHELF),
    ]),
    16: ("Tracks R",   "16",  80,   OFF_LPF, [     # was Keys R
        FLAT(),
        FLAT(),
        B(-3,  315, 2.0, BELL),
        B(-2,  200, 1.5, SHELF),
    ]),
    17: ("Click",      "17",  80,   OFF_LPF, [     # was Tracks L
        FLAT(),
        FLAT(),
        FLAT(),
        FLAT(),
    ]),
    18: ("Video",      "18",  80,   OFF_LPF, [     # was Tracks R
        FLAT(),
        FLAT(),
        FLAT(),
        FLAT(),
    ]),
    19: ("Ziggy Vox",  "19", 100,   15000, [       # was Click
        B(+3,  8000, 0.7, SHELF),
        B(+3,  3000, 1.2, BELL),
        B(-4,   300, 2.0, BELL,  deq=True, thr=-16, atk=0.010, rel=0.080),
        B(-4,   200, 1.5, SHELF),
    ]),
    20: ("Lua Vox",    "20", 100,   15000, [       # was Video
        B(+2,  8000, 0.7, SHELF),
        B(+3,  3000, 1.2, BELL),
        B(-4,   300, 2.0, BELL,  deq=True, thr=-16, atk=0.010, rel=0.080),
        B(-4,   200, 1.5, SHELF),
    ]),
    21: ("Brady Vox",  "21", 100,   15000, [       # was Ziggy Vox
        B(+2,  8000, 0.7, SHELF),
        B(+3,  3500, 1.2, BELL),
        B(-4,   300, 2.0, BELL,  deq=True, thr=-16, atk=0.010, rel=0.080),
        B(-4,   200, 1.5, SHELF),
    ]),
    22: ("Key Vox",    "22", 100,   15000, [       # was Lua Vox
        B(+2,  8000, 0.7, SHELF),
        B(+3,  3000, 1.2, BELL),
        B(-4,   300, 2.0, BELL,  deq=True, thr=-16, atk=0.010, rel=0.080),
        B(-4,   200, 1.5, SHELF),
    ]),
    23: ("Brian Vox",  "23", 100,   15000, [       # was Brady Vox; now in patcher range
        B(+3,  8000, 0.7, SHELF),
        B(+3,  3000, 1.2, BELL),
        B(-4,   300, 2.0, BELL,  deq=True, thr=-16, atk=0.010, rel=0.080),
        B(-4,   200, 1.5, SHELF),
    ]),
    24: ("John Vox",   "24", 100,   15000, [       # was Key Vox; now in patcher range
        B(+3,  8000, 0.7, SHELF),
        B(+3,  3000, 1.2, BELL),
        B(-4,   300, 2.0, BELL,  deq=True, thr=-16, atk=0.010, rel=0.080),
        B(-4,   200, 1.5, SHELF),
    ]),
    25: ("Test",       "25", 100,   OFF_LPF, [     # was Brian Vox; now Test / SM57
        B(+1,  8000, 0.8, SHELF),
        B(+2,  5000, 1.2, BELL),
        B(-4,   315, 2.0, BELL,  deq=True, thr=-16, atk=0.010, rel=0.080),
        B(-3,   200, 1.5, SHELF),
    ]),
}
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Apply Seals and Crofts 2 (v2) config to Q225 .ses template.")
