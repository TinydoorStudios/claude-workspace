...
The same day, the app-only Microsoft Graph connector that sends as `Production@3cdc.org` sent its first real mail through n8n — a 202, after three executions that all failed identically. Worth writing down because the debugging went wrong in an instructive way. The workflow used `responseMode: onReceived`, which answers the caller "Workflow was started" and then discards whatever happens next, so a node failing every single time looked like a success from outside. Three separate theories got chased on that bad signal. What actually settled it in one step was reading the error out of `execution_data` in n8n's own Postgres: `EAUTH`, client authentication failed, meaning it never reached Graph at all. The cause was the credential holding the client secret **ID** where the secret **Value** belongs — a UUID versus the ~40-character string Azure shows exactly once at creation. Microsoft's own error text names this case. **Read the execution error from the database before theorizing**, and prefer `responseMode: lastNode` with `fullResponse`/`neverError` on any workflow whose result matters.

A second n8n behaviour surfaced alongside it: webhook routes register noticeably after `/healthz` starts answering 200, so firing a request immediately after a restart returns 404 from a workflow that is in fact fine. Both gotchas now live in the article and in [Active Projects](/active-projects), which gained a Band Advance entry it had been missing entirely.

## 2026-08-14 — ESP NDI-audio nightly recycle had been dead for six weeks

The [ESP Magewell RX nightly recycle](/sop-esp-magewell-rx-recycle) stopped refreshing the two FSQ audio NDI receivers back on 2026-06-29 and nobody noticed until the audio kept going stale. Root cause: a 2026-06-28 edit on the ESP box re-saved the PowerShell script in an encoding that mangled its em-dash characters into mojibake, breaking the string terminators. A script that won't parse exits 1 and writes nothing to its log — so the task kept firing nightly and doing nothing, with the only trace a log frozen in June. Every dash is now a plain ASCII hyphen and the script is ASCII-only; don't edit it in an editor that reintroduces smart dashes.

Fixing that surfaced a second bug that had never worked: the connection-verify step read the wrong key. `rx/live-info` returns the channels under `gst` (matches Magewell's published schema), but the script only knew `channels/rx-channels/rx/data/list`, so every recycle false-failed even with both streams live. `gst` is now first in the key list. Third, the scheduled task had always been registered "Interactive only," so it would have died on any reboot or logoff regardless — re-registered to run as SYSTEM, "whether logged on or not." All three fixed in the canonical script, the wiki asset copy, and the box; live-verified end to end (both streams verify CONNECTED, exit 0; SYSTEM task run returns 0). Research confirmed the off/on toggle is the only reconnect mechanism Magewell's API offers — there's no auto-reconnect setting to lean on instead. Still open: the failure webhook is unset, so a real future failure alerts nobody.

## 2026-08-08 — Four mic-library corrections, and a shorthand rule that was backwards

- **mic-library:** SM81 marked **×3** — the row had no quantity marker while every other multiple in the file is marked explicitly, so it read as one mic and cost a question round on the FSQ 2nd Wind build (three channels called for an SM81). Confirmed by Brian.
- **mic-library:** new **"408" shorthand rule** section. A bare "408" is ALWAYS the Electro-Voice N/D 408, on any source including a snare; the Lauten LS-408 is "the Lauten" or "snare mic". This corrects a root-CLAUDE.md rule that claimed the opposite and had cost a question round on two consecutive FSQ builds. Both mic articles updated to match.
- **mic-library:** Audix D6 row refined against Tape Op's measured review — low peak at 60Hz, **two** high peaks at 4kHz AND 10kHz, midrange ~750Hz sitting ~15dB down. The old single "~5kHz" figure falls between the two real peaks and catches neither. Added the floor-tom application and the warning that on a tom the 60Hz peak lands in the kick/bass lanes.
- **eq-starting-points:** four new rows, all from the FSQ 2nd Wind build where their absence produced THIN verdicts — synth bass (DI), **bass amp XLR out post-EQ**, **direct feed from a modeller or emulated amp out**, **sampling pad**, and **backing-track playback**.
- Note: the fifth staged write-back (SM57 presence peak 3–5kHz → 6–7kHz) was found ALREADY APPLIED, corrected 2026-07-26. No change needed.
...
The read corrected two things in [Audiority Echoes T7E mkII](/fx-echoes-t7e). The SWITCH head matrix had positions 8 and 9 reversed — position 8 is Heads 1+3 and position 9 is Heads 2+4, which is the plugin's own label list, extracted from the binary and confirmed against the head-configuration block every preset file stores. And the article describes separate Bass and Treble knobs while the preset files store a single `Tone` value; flagged rather than silently rewritten, since the UI hasn't been checked.

`.aup` files turn out to be plain XML — one element, every parameter a 0–1 attribute — so the Echoes preset library reads, diffs and scripts cleanly. Worth remembering next to the FabFilter note in the main context file: `.ffp` is proprietary binary and can only be saved from inside the plugin, but Audiority presets are open text.

Two things stayed open rather than being guessed. The Selector (echo-mode) mapping is stored unlabeled and the 2026-07-11 reading and the 2026-07-31 reading disagree — details and a 30-second in-plugin test are in [Open Questions](/questions). And the nine presets built 2026-07-11 in `~/Documents/Claude/Echoes T7E Presets/` were never installed into the plugin; all nine store Noise at 100% and wet Volume near 25%, which does not match how the factory presets scale, so they are documented as suspect rather than promoted.
...
## 2026-07-30 — Shure PG52 added to the locker · Audix D4 row corrected · gallery-wipe bug fixed

Three things came out of the **Repertoire (Sound The Alarm) FSQ 2026-08-01** deep build.

**1. Shure PG52 researched and added.** Brian's instruction was "deep search this and add it to the locker." Full NEW-MIC-WORKFLOW run: `mic_data.json` record → `/mic-shure-pg52` page + reference PDF + gallery tile, plus rows in the mic-library Dynamics and Mic Character tables, `Memorial Hall/mic_inventory.xlsx`/`.csv`, the project CLAUDE.md shorthand table, and both `Code/ShowBuilder/knowledge/mics.json` (full record) and `Code/Patchbay/knowledge/mics.json` (slim). Specs from Shure's own PG52 Product Specifications (©2008, spec-sheet PDF read directly): cardioid dynamic, 30 Hz–13 kHz, 300 Ω, −55 dBV/Pa, neodymium, internal shock mount, 470 g, discontinued and superseded by the PGA52. The published curve humps at 60–100 Hz, dips broadly through 200–800 Hz, rises at 4–5 kHz and rolls off hard above 10 kHz.

The finding worth keeping: it is sold as a kick mic and the forums are blunt that it is a step below a Beta 52 there — but the same forums consistently name **bass cabinets** as the one place it punches above its price (Gearspace, HomeRecording, TalkBass), because that curve is the right shape for a cab. It arrived on this show on a bass cab, which is exactly right. EQ rules recorded: never boost the 60–100 Hz hump (own the low end by letting it through), never cut into the 200–800 Hz dip (put a box cut at ~160 Hz on the shoulder instead), and trim the 4–5 kHz rise rather than boosting near it. Photo still pending — the mic is discontinued so there is no manufacturer product shot, and the page shows the navy placeholder until Brian drops one in.

**2. Audix D4 character row corrected.** The row said the D4 "reaches 35Hz" and has "less upper-mid attack — may need a touch of click." RecordingHacks, reading Audix's current published chart, gives the opposite at both ends: **+6 dB at 5 kHz** with a secondary peak above 10 kHz, and a **low-frequency rolloff below 70 Hz**. The old row would have put a click boost directly onto a baked peak. Brian's call on the Repertoire build was option (c) — go with the research and fix the row — so the tendency is now "trim the peak ~5000 Hz, never boost there; restore weight ~80 Hz rather than expecting sub reach."
